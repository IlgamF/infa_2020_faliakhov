...
def change_ball(n):
    ...
    r = randint(R_min, R_max)
    color = COLORS[randint(0, 5)]
    Balls[n] = [x, y, v_x, v_y, r, color]                     #список координат, к которому обращаемся позже
...
def count_walls():
    ...
    for i in range(len(Balls)):
        if abs(Balls[i][0]) <= Balls[i][4]:
            Balls[i][2] = randint(max_speed//10, max_speed)               #используем координаты из списка выше
...
def game():
    ...
    turtle_x = 400
    turtle_y = 300                                                        #большая черепашка
    turtle_x_speed = 0
    turtle_y_speed = 0
    turtle_speed = 20
    ...
    while not finished:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if len(playlist) > 0:
                    pygame.mixer.music.load(playlist.pop())                               #музыка
                    pygame.mixer.music.play()
                    if len(playlist) == 1:
                ...
                for i in range(len(Balls)):
                    if Balls[i][4] ** 2 > (Balls[i][0] - event.pos[0]) ** 2 + (Balls[i][1] - event.pos[1]) ** 2:               #проверка попадания в шар                
                        points += 1                                                                            
                        screen.fill(BLACK)
                        change_ball(i)
                ...
                if turtle_exists:
                    if (mouse_x - turtle_x)**2 + (mouse_y - turtle_y)**2 < radius // 2:                      #проверка попадания в черепашку
        ...
        if turtle_exists:
            ...
            if turtle_y <= 0 or turtle_y >= screen_y_size:                       #движение черепашки на границах экрана
        ...
        screen.blit(time_text, (screen_x_size - 2 * time_text.get_width() / 2, time_text.get_height() / 2))                       #вывод таймера на экран
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_SPACE:
                    ...
                    screen.fill(BLACK)
                    screen.blit(greeting_text_3, (screen_x_size / 2 - greeting_text_3.get_width() / 2,                #набор имени игрока + начальный текст
...
def ending():
    ...
    if is_turtle_catched:
        ending_text = greeting_font.render("Поздравляю!!! Вы поймали черепашку. " +
                                             "У Вас" + ' ' + str(points) + ' ' + "очков", 0, WHITE)
    else:
        ending_text = greeting_font.render("У Вас " + str(points) +
                                           " очков", 0, WHITE)

    screen.blit(ending_text, (screen_x_size / 2 - ending_text.get_width() / 2,                                       #окончание игры
...
lines = [line.rstrip('\n') for line in file]                                              #сортировка результатов по убыванию 
j = []
